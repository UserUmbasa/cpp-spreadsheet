...
void Sheet::SetCell(Position pos, std::string text) {
    ...
    //ищем в таблице указатель на позицию
    const auto& cell = cells_.find(pos);
    
    if (cell == cells_.end())
    {
        //если указателя нету, создадим пустую ячейку в таблице
        cells_.emplace(pos, std::make_unique<Cell>(*this));
    }

    //присвоим позиции значение ячейки (вызовем у нее метод Set, там же чекнем на кэш и циклы)
...
Size Sheet::GetPrintableSize() const {
    ...
    Table cells_;*/
    //едем по таблице ,расширяя окно (только если ячейка не пустая)
